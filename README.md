...
| Reactions    | Players send each other emoji mid-game. The pop springs out of the sender's strip on every screen watching, including spectators'.                                                                                                                                                                                                                                                                                                                    |
| Music        | Endgame tension, time trouble, and win or loss stings, mixed under the chiptune sounds. On by default, switchable per player.                                                                                                                                                                                                                                                                                                                         |
...
Players can send each other one of eight emoji during a live game. The reaction rides on the room document as a single `reaction` field rather than its own collection, so it reaches both players and every spectator through the listener that is already open, with no extra reads and nothing to clean up afterwards.

Both players share that one slot, which is what lets `firestore.rules` do the policing: it checks the emoji against its own copy of the list (a test keeps the two in step), requires a server timestamp, requires the counter to move on by exactly one, and holds everyone to 1.5 seconds between reactions. A write that loses the race is dropped rather than retried.

The pop springs out of the sender's player strip and drifts towards the middle of the screen — down from the top strip, up from the bottom one — so it never crosses the header or the tab bar. Everything animates on transform and opacity. Light effects mode and reduced motion keep the pop and drop the flight. The tray opens in the browser's top layer (the toolbar sits inside a blurred panel, and a blur would otherwise pin it there), hanging under the button on big screens and docking above the tab bar on phones.

## Music

Four cues play at the moments that carry a game, over the chiptune blips in `sound.service.ts`:

| Cue            | When it plays                                                                                   |
| -------------- | ----------------------------------------------------------------------------------------------- |
| `tension`      | The board is down to an endgame (24 pawns of material or less), looping until something changes |
| `time`         | Your clock drops under 30 seconds, whatever is left on the board                                |
| `win` / `loss` | A decisive ending, over the finale cinematic                                                    |

Music is on by default, has its own button in the game toolbars, and muting all sound silences it too. Draws end in silence. Tracks are only fetched once a game first needs them, and the service worker keeps them for offline bot games.

The source recordings aren't in the repo. `npm run audio:music -- "D:\Videos\Musics"` rebuilds `public/audio/` from them: each cue is cut to the part the game uses, normalized, downmixed to mono and encoded as Opus at 32 kbps (85-222 kB each), with an AAC copy for browsers without Ogg/Opus. The two loops are crossfaded end to start so they repeat without a seam.
...
| ------------------- | ------- | ------------------------------------------------------------------ |
| `sky-wide-1080.mp4` | 20.9 MB | Laptops and desktops where the sky is over ~820 device pixels tall |
| `sky-wide-720.mp4`  | 10.1 MB | Smaller landscape screens and slow connections                     |
| `sky-tall-1080.mp4` | 11.7 MB | Phones and portrait tablets (4:5 center crop)                      |
| `sky-tall-720.mp4`  | 5.8 MB  | Small portrait screens and slow connections                        |

All are AVC (H.264 High, 8-bit), preset slow, CRF 27 at 1080p and 28 at 720p. The wrap geometry is derived in the script: the horizon sits 80% down the frame, and the view stays inside the source’s vertical field of view, so the top of the frame never samples past the render. The choice is made once in `src/app/features/home/home-sky.ts`, and switches crops when a device rotates. The videos are streamed, not cached by the service worker; the `sky-*.jpg` still frames are.
